...
from statetrackingutils import SlotFillingIntentDetectionModelOutput
import torch

class IntentClassifier(nn.Module):
    def __init__(self, in_size, out_size, dropout_prob=.5):
...
class BERTDSTI(BERTDSTMain):
    ...
    def post_init(self, info, *args, **kwargs):
        ...
        self.intent_pooler_layer = Pooler(pooler_index=1, hidden_size=self.config.hidden_size)
        self.intent_classifier = IntentClassifier(self.config.hidden_size, len(info['all_intents']))

        print(f"Loaded BERT-DSTINTENT model with intents: {self.intent_keys}", flush=True)
    ...
    def forward(
        ...
        **kwargs
        ...
        if get_output_for_metrics_end:
            ...
            loss = self.compute_loss((model_out, gate_logits, span_logits, intent), extra_info, accel)
    ...
    def compute_loss(self, model_out, ground_truth, accel, *args, **kwargs):
        intent_loss_percentage = .4
        slot_filling_loss_percentage = .6

        slot_filling_loss = super().compute_loss(model_out, ground_truth, accel, args, kwargs)
        loss_fn = nn.CrossEntropyLoss()
        intent_logits = model_out[3]
        intent_gt = [self.intent_keys.index(item.intent.lower()) for item in ground_truth]
        intent_loss = loss_fn(intent_logits, accel.to_device_single(accel.prepare_data(torch.tensor(intent_gt))))

        return intent_loss_percentage * intent_loss + slot_filling_loss_percentage * slot_filling_loss
    ...
    def get_output_for_metrics(
        ...
        **kwargs
        ...
        outs = super(BERTDSTI, self).get_output_for_metrics(model_out, items, len(extra_info), extra_info, should_return=True)
        for bs_index in range(len(extra_info)):
            outs[bs_index].intents = []
            outs[bs_index].set_intent(self.intent_keys[torch.max(model_out[3][bs_index], dim=0).indices])
