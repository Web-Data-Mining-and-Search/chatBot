...
class TypicalTransformerEncoderQAInputFunction:
    ...
    def __call__(self, dataset=None, item=None, index=None, question='', context='', tokenizer=None, **kwargs):
        if dataset is not None:
            tokenizer = dataset.tokenizer
            question = item.question
            context = item.context

        return self.typical_transformer_encoder_qa_input_function(tokenizer, question, context, **kwargs)


    def typical_transformer_encoder_qa_input_function(self, tokenizer, question, context, **kwargs):
...
class TypicalTransformerEncoderQAInputFunctionWithExtraSlotInfo:
    def __init__(self, max_sequence_len: int, to_prepend: str, **kwargs):
        import json
        with open(f'./eval/dst_eval/eval-data/multiwoz21/validation.json', 'r') as json_data_eval:
            self.data_eval = json.load(json_data_eval)
        with open(f'./eval/dst_eval/eval-data/multiwoz21/test.json', 'r') as json_data_test:
    ...
    def __call__(self, dataset=None, item=None, index=None, question='', context='', tokenizer=None, **kwargs):
        ...
        inp_func = self.typical_transformer_encoder_qa_input_function(tokenizer, question, final_str + ' ' + context, **kwargs)
        if item.utterance_index > 0 and dataset.split != 'train':
            if dataset.split == 'test':
                vals = self.data_test[item.dialogue_id][str(item.utterance_index - 1)]['turn_belief']
            else:
                vals = self.data_eval[item.dialogue_id][str(item.utterance_index - 1)]['turn_belief']
            ...
            if item.answer and item.answer != ['']:
                s, e = statetrackingutils.find_span(item.answer, inp_func.encodings[0].tokens)
                if s != -1 and e != -1:
...
class T5PPromptedInputFunction:
    def __init__(self, max_sequence_len: int, to_prepend_a: str, to_prepend_b: str, **kwargs):
        self.max_sequence_len = max_sequence_len
