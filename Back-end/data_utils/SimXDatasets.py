...
class SimXDataset(SFDataset.SFDataset):
    def __init__(
        ...
        super().__init__(loc=loc, name=name, split=split, tokenizer=tokenizer, max_sequence_len=max_sequence_len,
                         toy=toy, force_len=force_len, shuffle_samples=shuffle_samples, create_empty=create_empty,
                         using_system_utterances=using_system_utterances, dropout=dropout,
                         context_window_length=context_window_length, negative_example_proportion=negative_example_proportion, model_input_function=model_input_function)
        self.loc = loc
        self.name = name
        self.split = split
        ...
        if self.toy and self.split == 'train':
            ...
            print(f'Shuffling data! Old number of dialogues: {len(data_raw)} New number: {int(force_len * len(data_raw))}')
            shuffle(data_raw)
        ...
        print(f"Slot-filling ({self.name}) dataset loaded... Using state: {self.use_dialogue_state}; Ignoring slots {self.ignore_keys}; Ignoring intents: {self.ignore_intents}; {len(self)} examples.", flush=True, end='\t')
        if self.split == 'train':
            print('All slots:', self.all_slots)
        if self.split == 'test':
            config.ignore_keys_validate_test = self.ignore_keys
        print(f'Note that it doesn\'t actually ignore any slot keys and instead simply passes them on to the model (or dataset transformation). They should ignore them instead.', flush=True)
        print(f"note 2: slot_dict = slot_dict_dialogue_state used to be slot_dict = deepcopy(slot_dict_dialogue_state)", flush=True)
    ...
    def generate_outputs_for_dsteval(self, name, split):
        ...
        for item in self.extra_info:
            ...
            if item.utterance_index > 0:
                out_file[str(item.dialogue_id)][str(item.utterance_index)] = deepcopy(prev)
            else:
                out_file[str(item.dialogue_id)][str(item.utterance_index)] = {'turn_belief': dict()}
            for slot in item.slots.values():
                if slot.slot_gate == 1:
                    out_file[str(item.dialogue_id)][str(item.utterance_index)]['turn_belief'][slot.slot_key] = 'dontcare'

                elif slot.slot_gate == 2:
                    slot_value = slot.slot_value

                    timepat = re.compile(r"(\d{1,2} : \d{1,2})|(\d{1,2}( )?[.] \d{1,2})") # unfortunately we need to do this for trade+mwoz compatibility (for now)
                    def time_repl(matchobj):
                        time = matchobj.string[matchobj.regs[0][0]: matchobj.regs[0][1]]
                        return time.replace(" ", "")
                    slot_value = re.sub(timepat, time_repl, slot_value)

                    out_file[str(item.dialogue_id)][str(item.utterance_index)]['turn_belief'][slot.slot_key] = slot_value
            prev = deepcopy(out_file[str(item.dialogue_id)][str(item.utterance_index)])
            out_file[str(item.dialogue_id)][str(item.utterance_index)]['turn_belief'] = [k + '~' + v for k, v in out_file[str(item.dialogue_id)][str(item.utterance_index)]['turn_belief'].items()]
                    ...
                    for i in range(len(turn['dialogue_state'])):
                        for ignore_slot in self.ignore_keys:
                            if ignore_slot in turn['dialogue_state'][i]['slot']:
                                actual_ignored_keys.add(turn['dialogue_state'][i]['slot'])
                else:
                    for i in range(len(turn['user_utterance']['slots'])):
                        for ignore_slot in self.ignore_keys:
                            if ignore_slot in turn['user_utterance']['slots'][i]['slot']:
                                actual_ignored_keys.add(turn['user_utterance']['slots'][i]['slot'])
                    for i in range(len(turn.get('system_utterance', {}).get('slots', []))):
                        for ignore_slot in self.ignore_keys:
                            if ignore_slot in turn['system_utterance']['slots'][i]['slot']:
                                actual_ignored_keys.add(turn['system_utterance']['slots'][i]['slot'])
    ...
    def process_data(self, data_raw):
        ...
        for dialogue in data_raw:
            ...
            if 'services' in dialogue and self.ignore_domains == dialogue['services'] and dialogue['services']:
                continue
            if len(self.force_domains) > 0 and not any([item in self.force_domains for item in dialogue['services']]): # if we're forcing domains and none of the domains are present
                continue
            for i, turn in enumerate(dialogue["turns"]):
                ...
                system_turn = self.system_token + ' ' + system_info["text"] if system_info is not None else ''
                entire_utterance = system_turn + ' ' + user_turn
                context = self._get_full_context(prev_state) if i > 0 else ''
                if self.lower_text:
                    context, entire_utterance = context.lower(), entire_utterance.lower()
                ...
                if self.use_dialogue_state:
                    slot_dict_dialogue_state = self.extract_dontcares(turn["dialogue_state"], prev_state["dialogue_state"], slot_dict_dialogue_state)
                    slot_dict_dialogue_state = self.extract_slot_info_from_dialogue_state(slot_dict_dialogue_state, prev_state["dialogue_state"], tokenized_text, turn['dialogue_state'])
                    slot_dict_dialogue_state = self.extract_nones(slot_dict_dialogue_state)
                    slot_dict = slot_dict_dialogue_state
                    slot_dict_dialogue_state = dict()
                else:
                    slot_dict = self.extract_dontcares(turn["dialogue_state"], prev_state["dialogue_state"], slot_dict)
                    if system_info is not None:
                        slot_dict = self.extract_slot_info(slot_dict, tokenized_text, system_info)
                    slot_dict = self.extract_slot_info(slot_dict, tokenized_text, user_info)
                ...
                extra_info.previous_state = prev_state["prev_state"]  # TODO previous state should just be the previous turn's slotfillinginputinfo
    ...
    def extract_slot_info_from_dialogue_state(self, slot_dict, prev_state, tokenized_text, dialogue_state):
        for slot in dialogue_state:
            try:
                tokenized_slot_value = self.tokenizer.tokenize(' ' + slot['value'].lower())
                span_start, span_end = statetrackingutils.find_span(
                    tokenized_text, tokenized_slot_value, end_token=self.tokenizer.pad_token
                )
            except Exception as e:
            ...
            if self._slot_in_state(slot, prev_state) and (span_start == -1 and span_end == -1):
                continue
            elif span_start == -1 and span_end == -1 and slot['slot'] in slot_dict and slot_dict[slot["slot"]].slot_gate == 1:
                ...
                slot_dict[slot["slot"]] = statetrackingutils.SingleSlotInputExample(
                    slot_key=slot["slot"],
                    slot_value=self.tokenizer.convert_tokens_to_string(tokenized_slot_value).strip(),
        ...
        for slot in info["slots"]:
            if 'value' in slot:
                ...
                tokenized_slot_value: list[str] = self.tokenizer.tokenize(
                        ' '.join(info["tokens"][slot["start"]: slot["exclusive_end"]])  # TODO this might not generalize
                    # but when we tried to run it with a BART tokenizer, things failed, because it turned 6:00 pm into 6:00pm
                )
                if all([len(item) == 1 for item in tokenized_slot_value]) and len(tokenized_slot_value) > 1:
                    # Common error: 6:00 pm -> [6, :, 0, 0, p, m]
                    print('If this message is flooding the console, you have some tokenization error.')
            ...
            slot_dict[slot["slot"]] = statetrackingutils.SingleSlotInputExample(
                slot_key=slot["slot"],
                slot_value=self.tokenizer.convert_tokens_to_string(tokenized_slot_value),
                tokenized_slot_value=tokenized_slot_value,
                slot_gate=2,
                slot_gate_str="value",
                start=span_start,
    ...
    def extract_dontcares(self, dialogue_state, prev_state, slot_dict):
        for slot_type in dialogue_state:
            if slot_type["value"] == "dontcare" and (
                len([item for item in prev_state if item["slot"] == slot_type["slot"]]) == 0 or not
                [item for item in prev_state if item["slot"] == slot_type["slot"]][0]["value"] == "dontcare"
            ):
                slot_dict[slot_type["slot"]] = statetrackingutils.SingleSlotInputExample(
